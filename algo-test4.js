...
function removeFirstAndLast(string) {
    ...
    newStr = newStr.slice(1,newStr.length-1).join(' ')
...
function deEmojify(emojiString) {
    const codes = [':)', ':D', '>(', '>:C', ':/', ':|', ':O', ';)', '^.^', ':(']
    let arr = emojiString.split('  ').map(x=>x.split(' ')).map(x=>x.map(y=>codes.indexOf(y)).join('')).map(x=>String.fromCharCode(x)).join('')
    return emojiString.length ? arr : ""
  }
